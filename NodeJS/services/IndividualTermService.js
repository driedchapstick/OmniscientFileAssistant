...
async function addCriteriaTerms(CriteriaID, AllTerms) {
  ...
  if (typeof AllTerms === "object") {
    ...
    await pool
      .request()
      .input("CriteriaID", sql.Int, CriteriaID)
      .input("TermID", sql.Int, AllTerms)
      .execute("AddCriteriaTerms");
  } else {
    console.log(
      "ERROR: The terms that were attempted to be added to CriteriaTerms was not an Object or String."
    );
    console.log("ERROR: Object == Multiple Terms    String == Single Term");
    console.log(
      "ERROR: The user must have submitted something purposefully incorrect or malicious."
    );
